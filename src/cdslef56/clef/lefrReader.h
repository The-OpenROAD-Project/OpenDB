...
/*
 * The reader initialization to start reading another LEF file. It must be
 * called before the 2nd, 3rd, etc call to lefrRead().
 */
...
/*
 * Change the comment character in LEF.  The normal character is '#'.   You
 * can change it to anything you want, but be careful.
 */
EXTERN void lefrSetCommentChar PROTO_PARAMS(( char c ));

/*
 * Allow the parser to upshift all names if the LEF file is case insensitive.
 * The default is no shift, so the user must do case insensitive matching.
 */
EXTERN void lefrSetShiftCase PROTO_PARAMS(( void ));

/*
 * Allow the user to change the casesensitivity anytime during parsing.
 * caseSen = 0, will turn off the casesensitivity caseSen != 0, will turn on
 * the casesensitivity
 */
 ...
 * time, and it will take effect on the next callback.  The lefi reader and
 ...
 * and will call the corresponding callbacks (lefrErrorCbkFnType and
 * lefrWarningCbkFnType) if they are supplied. The messages are formatted
 * before calling the callbacks.
 */

/*
 * EXTERN void lefrError PROTO_PARAMS(( const char *, ... )); EXTERN void
 * lefrWarning PROTO_PARAMS(( const char *, ... )); EXTERN void lefrVError
 * PROTO_PARAMS(( const char *, va_list )); EXTERN void lefrVWarning
 * PROTO_PARAMS(( const char *, va_list )); EXTERN int
 * lefrGetCurrentLineNumber PROTO_PARAMS(( void )); EXTERN const char
   * *lefrGetCurrentFileName PROTO_PARAMS(( void ));
 */
 ...
 * 
 * The lefrDesignStart and lefrDesignEnd callback is only called once. Other
 ...
 * 
 * All callbacks pass the user data pointer provided in lefrRead() or
 * lefrSetUserData() back to the client; this can be used by the client to
 * obtain access to the rest of the client's data structures.
 * 
 * The user data pointer is obtained using lefrGetUserData() immediately prior
...
void            lefrSetReadFunction(LEFI_READ_FUNCTION);
