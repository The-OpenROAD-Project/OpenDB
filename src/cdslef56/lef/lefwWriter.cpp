...
int lefwLayerWidth(double minWidth) {
  ...
  if (lefwState != LEFW_LAYER_START &&
     lefwState != LEFW_LAYER) return LEFW_BAD_ORDER;
  if ((!lefwIsImplant) && (!lefwIsCut)) return LEFW_BAD_DATA;
...
int lefwLayerSpacingAdjacent(double spacing, 
...
int lefwLayerSpacingCenterToCenter(double spacing) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYER_START &&
     lefwState != LEFW_LAYER) return LEFW_BAD_ORDER;
  if (!lefwIsCut) return LEFW_BAD_DATA;  // only cut calls this func
...
int lefwLayerRoutingDiagPitchXYDistance(double xDistance, double yDistance) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
      lefwState != LEFW_LAYERROUTING) return LEFW_BAD_ORDER;
  if (!lefwIsRouting) return LEFW_BAD_DATA;  // only routing calls this func
...
int lefwLayerRoutingMinstepWithOptions(double distance, const char* rule,
                                       double maxLength) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
      lefwState != LEFW_LAYERROUTING) return LEFW_BAD_ORDER;
...
int lefwViaResistance(double resistance) {
  ...
  if (lefwViaHasVal) return LEFW_ALREADY_DEFINED;  // either Resistance or
                                                   // ViaRule has defined
...
int lefwViaLayerPolygon(int num_polys, double *xl, double* yl) {
  int i;
  int polyCount;
  ...
  if (lefwState != LEFW_VIA_START && lefwState != LEFW_VIA)
      return LEFW_BAD_ORDER;
  if (!lefwDidLayer) return LEFW_BAD_ORDER;
  if (num_polys < 4) return LEFW_BAD_DATA;
...
int lefwViaViarule(const char* viaRuleName, double xCutSize, double yCutSize,
...
int lefwViaViaruleOffset(double xBotOffset, double yBotOffset,
...
int lefwEndVia(const char* viaName) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_VIA_START && lefwState != LEFW_VIA &&
      lefwState != LEFW_VIAVIARULE) return LEFW_BAD_ORDER;
  fprintf(lefwFile, "END %s\n\n", viaName);
...
int lefwSiteRowPatternStr(const char* siteName, const char* orient) {
  ...
  if (lefwState != LEFW_SITE) return LEFW_BAD_ORDER;
  if (!siteName || siteName == 0 || *siteName == 0) return LEFW_BAD_DATA;
  if (!orient || orient == 0 || *orient == 0) return LEFW_BAD_DATA;
  fprintf(lefwFile, "   ROWPATTERN %s %s ;\n", siteName, orient);
  lefwLines++;
  lefwState = LEFW_SITE;
  return LEFW_OK;
...
int lefwStartMacroDensity(const char* layerName) {
  ...
  if (lefwState != LEFW_MACRO_START &&
     lefwState != LEFW_MACRO) return LEFW_BAD_ORDER;
  if (lefwIsMacroDensity) return LEFW_BAD_ORDER;
  if (!layerName || layerName == 0 || *layerName == 0) return LEFW_BAD_DATA;
...
int lefwMacroDensityLayerRect(double x1, double y1, double x2, double y2,
                              double densityValue) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (! lefwDidInit) return LEFW_BAD_ORDER;
  if (! lefwIsMacroDensity) return LEFW_BAD_ORDER;
...
int lefwMacroPinPortLayerPath(int num_paths, double* xl, double* yl,
  ...
  if (numX || numY || spaceX || spaceY) {
     ...
     for (i = 0; i < num_paths; i++) {
        ...
        else fprintf(lefwFile, "%.11g %.11g ", xl[i], yl[i]);
        numItem++;
     }
  }
  fprintf(lefwFile, ";\n");
