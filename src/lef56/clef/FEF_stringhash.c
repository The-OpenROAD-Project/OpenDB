 ...
 * 
 * There are only three functions: table = lef_make_stringtab(store); bool =
 * lef_present_in_stringtab(table, string, &val) insert(table, string, ptr);
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
struct htable {
  ...
  char   *pool;			/* points into the first block on the above
				 * list */
...
/*
 * The following functions come in two versions - one for exact match, which
 * is faster but case sensitive, and one for case insensitive.
 */
static int 
...
/*
 * NOTE: rehashval MUST return an odd number, or the search algorithm may
 * infinite loop.  This is because the rehash value must be relatively prime
 * to the table size to ensure all slots are searched before we loop back to
 * the original.  Since the table size is a power of 2, we need rehash to be
 * odd to guarentee no common factors
 */
static int 
rehashval(const char *str)
{
  const char *p;
...
int 
lefcasecmp(const char *str1, const char *str2)
{

  /*
   * NT doesn't have this function
   *//* return 0 if the strings match (case insensitive) */ int l = strlen(str1);
...
int 
lef_present_in_stringtab(void *hdl, const char *str, int **ret)
{
...
int 
lef_modify_stringtab(void *hdl, const char *str, void *new_val)
{
  HTABLE *htab = (HTABLE *) (hdl);
...
const char *
lef_make_string_in_stringtab(HTABLE * htab, const char *str, void *val)
{
  int     h;
  ...
  if (htab->table[h].string) {
    int     r = (htab->rehash_fn) (str);	/* only compute the rehash if
						 * needed */
  ...
  htab->has++;
  htab->table[h].value = val;
  if (htab->alloc == TABLE_ITSELF_ALLOCS) {	/* find an entry from the
						 * pool  */
...
void 
lef_add_to_stringtab(void *hdl, const char *str, void *val)
{
  HTABLE *htab = (HTABLE *) (hdl);
...
static const char *
stringtab_alloc_string(HTABLE * htab, const char *str)
{
  char   *addr;
...
void 
lef_destroy_stringtab(void *handle)
{
...
int 
lef_active_hashtables(const char *indent)
{
  int     i;
...
int 
addAlias(const char *key, const char *data)
{
  HTABLE *h = (HTABLE *) alias_set;
