...
liberty_attribute_value *sy_attr_calloc(void) {
  if (attr_memN >= 1024) {
    odb::notice(0, "Error, my_attr_calloc overflow\n");
    exit(-1);
  }
...
void sy_attr_free(liberty_attribute_value *v) {
  int j = v - attr_memV;
  if (j < 0 || j >= attr_memN) {
    odb::notice(0, "Error, my_attr_free j=%d\n", j);
    exit(-1);
  }
  attr_memU[j] = 0;

  if (j == attr_memN - 1) {
    while (j >= 0 && !attr_memU[j])
      j--;
    attr_memN = j + 1;
  }
}
...
void sy_double_used(int n) {
  d_memN += n;
  if (d_memN > 1024) {
    odb::notice(0, "Error, overflow of doubles\n");
    exit(-1);
  }
}

void sy_double_free_all(void) { d_memN = 0; }
