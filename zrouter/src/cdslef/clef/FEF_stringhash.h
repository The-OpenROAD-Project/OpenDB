 ...
 * FEF_stringhash.h
 * 
 * TODO  The 'void *handle' arguments should really be 'HTABLE *' but this
 * requires changing all uses.  For now make_stringtab will return an HTABLE
 * *, which can be assigned to the 'void *' that is used in old code or a
 * 'HTABLE *' pointer in new code. The 'void *' argument will accept both
 * 'void *' and 'HTABLE *' arguments.
 */
typedef struct htable HTABLE;

/*
 * Some constants for calling lef_make_stringtab. These should be enumerated
 * types, but its a pain to change now.
 */
enum table_alloc {
  TABLE_CALLER_ALLOCS, TABLE_ITSELF_ALLOCS

};

enum table_case {
  TABLE_IGNORE_CASE, TABLE_USE_CASE

};

extern HTABLE  *
lef_make_stringtab(enum table_alloc alloc, const char *comment,
 ...
 * otherwise it adds it.  If the caller is allocating the string, then the
 * caller will probably have to free the incoming string if the return value
 * indicates the table is using an existing string.
 */
...
/*
 * lef_modify_stringtab modifies the value (an arbitrary int) associated with
 * this string and return TRUE if the string is present. If the string isn't
 * present, the function does nothing and returns FALSE.
 */
