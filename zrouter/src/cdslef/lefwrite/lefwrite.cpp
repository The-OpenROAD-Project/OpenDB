...
int main(int argc, char** argv) {
  ...
  CHECK_STATUS(status);
  status = lefwUseMinSpacing("PIN", "ON");
  CHECK_STATUS(status);
  status = lefwClearanceMeasure("EUCLIDEAN");
  CHECK_STATUS(status);
  // status = lefwClearanceMeasure("MAXXY");
  // CHECK_STATUS(status);
  status = lefwNewLine();
  CHECK_STATUS(status);
  ...
  CHECK_STATUS(status);
  status = lefwSlotWireWidth(3.5);
  CHECK_STATUS(status);
  status = lefwSlotWireLength(4.5);
  CHECK_STATUS(status);
  status = lefwSlotWidth(5.6);
  CHECK_STATUS(status);
  status = lefwSlotLength(3.2);
  CHECK_STATUS(status);
  status = lefwMaxAdjacentSlotSpacing(5.5);
  CHECK_STATUS(status);
  status = lefwMaxCoaxialSlotSpacing(6.5);
  CHECK_STATUS(status);
  status = lefwMaxEdgeSlotSpacing(5.0);
  CHECK_STATUS(status);
  status = lefwSplitWireWidth(3.3);
  CHECK_STATUS(status);
  ...
  CHECK_STATUS(status);
  status = lefwLayer(0.7, "RX");
  CHECK_STATUS(status);
  status = lefwLayerSpacingAdjacent(0.22, 0.25);
  CHECK_STATUS(status);
  ...
  // VIARULE with GENERATE & ENCLOSURE
  ...
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("RX", 10.0, 2.2, 6, 6.5, 6.5, 6.5);
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("PC", 10.0, 2.2, 0, 0, 6.5, 0);
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("M1", 10.0, 2.2, 0, 6.5, 0, 0);
  CHECK_STATUS(status);
  status = lefwStartVia("nd1VARX0", NULL);
  CHECK_STATUS(status);
  status = lefwViaResistance(0.2);
  CHECK_STATUS(status);
  status = lefwViaForeignStr("IN1X", 0, 0, "N");
  CHECK_STATUS(status);
  status = lefwViaLayer("RX");
  CHECK_STATUS(status);
  ...
  */
  ...
  CHECK_STATUS(status);
  status = lefwMacroPinForeign("GROUND", 4, 7, 6);
  CHECK_STATUS(status);
  status = lefwMacroPinForeignStr("VSS", 4, 7, "W");
  CHECK_STATUS(status);
