...
int versionCB(lefwCallbackType_e c, lefiUserData ud) {
  ...
  status = lefwVersion(5, 3);
  CHECK_STATUS(status);
  return 0;
}

int caseSensCB(lefwCallbackType_e c, lefiUserData ud) {
  int status;

  checkType(c);
  if ((int)ud != userData) dataError();
  status = lefwCaseSensitive("ON");
  CHECK_STATUS(status);
  return 0;
}

int noWireCB(lefwCallbackType_e c, lefiUserData ud) {
  int status;

  checkType(c);
  if ((int)ud != userData) dataError();
  status = lefwNoWireExtensionAtPin("ON");
  CHECK_STATUS(status);
  return 0;
}
...
int layerCB(lefwCallbackType_e c, lefiUserData ud) {
  ...
  CHECK_STATUS(status);
  status = lefwLayerRoutingSpacing(0.6, 0.1, 9);
  CHECK_STATUS(status);
  ...
  CHECK_STATUS(status);
  status = lefwLayerRoutingSpacing(0.6, 0, 0);
  CHECK_STATUS(status);
  ...
  CHECK_STATUS(status);
  status = lefwLayerRoutingSpacing(0.9, 0, 0);
  CHECK_STATUS(status);
...
int viaCB(lefwCallbackType_e c, lefiUserData ud) {
  ...
  CHECK_STATUS(status);
  status = lefwViaTopofstackonly();
  CHECK_STATUS(status);
  status = lefwViaForeign("IN1X", 0, 0, -1);
  CHECK_STATUS(status);
...
int viaRuleCB(lefwCallbackType_e c, lefiUserData ud) {
  ...
  status = lefwViaRuleLayer("M1", "VERTICAL", 9.0, 9.6, 4.5, 0);
  CHECK_STATUS(status);
  status = lefwViaRuleLayer("M2", "HORIZONTAL", 3.0, 3.0, 0, 0);
  CHECK_STATUS(status);
  ...
  CHECK_STATUS(status);
  status = lefwViaRuleGenLayer("M1", "VERTICAL", 0.1, 19, 1.4, 0);
  CHECK_STATUS(status);
  status = lefwViaRuleGenLayer("M2", "HORIZONTAL", 0, 0, 1.4, 0);
  CHECK_STATUS(status);
...
int nonDefaultCB(lefwCallbackType_e c, lefiUserData ud) {
  ...
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("RX", 10.0, 2.2, 6);
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("PC", 10.0, 2.2, 0);
  CHECK_STATUS(status);
  status = lefwNonDefaultRuleLayer("M1", 10.0, 2.2, 0);
  CHECK_STATUS(status);
...
// CROSSTALK
int crosstalkCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
...
// NOISETABLE
int noiseTableCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
  double *holder;
...
// CORRECTIONTABLE
int correctTableCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
  double *holder;
...
// SPACING
int spacingCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
...
int dielectricCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
...
// IRDROP
int irdropCB(lefwCallbackType_e c, lefiUserData ud) {
  int    status;
