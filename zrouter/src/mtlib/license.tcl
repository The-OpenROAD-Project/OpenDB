...
itcl::class LLicenseManagerTimeBomb {
    inherit LLicenseManager
    # -------------------------------------------------- Public Variables --
    public variable expires "3/31/2023"
	...
	set dt  [expr $exp - $now]
	if { $dt <= 0 } {
	    set R(msg) "Nefelus RC Extraction license expired (at $expires)"
	    return 1
	}
	if { ($warningtime) && ($dt <= $warningtime) } {
	    set R(msg) "warning:Nefelus RC Extraction license will expire soon ($expires)"
	    return -1
	}
	return 0
    }
