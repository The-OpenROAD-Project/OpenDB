...
itcl::class LRPC2 {
    ...
    public method SendStreamTo {_addr scmd rcmd} {
        # -- parse addr
        foreach {_pport _ip} [split $_addr @] { break }
        set npeer [rpc_connect $_ip $_pport 5]
        fconfigure $npeer -buffering full -encoding binary -translation binary
        puts stderr "streaming to $_addr ($scmd) ($rcmd)"
        if { [catch {rpc_get_stream $npeer $scmd $rcmd} rc] } {
            catch {close $npeer}
            return -code error "$rc"
        }
        puts stderr "finished streaming"
        return $rc
    }
    public method GetStreamFrom {_addr scmd rcmd} {
        # -- parse addr
        foreach {_pport _ip} [split $_addr @] { break }
        set npeer [rpc_connect $_ip $_pport 5]
        fconfigure $npeer -buffering full -encoding binary -translation binary
        puts stderr "streaming from $_addr ($scmd) ($rcmd)"
        if { [catch {rpc_get_stream $npeer $scmd $rcmd} rc] } {
            catch {close $npeer}
            return -code error "$rc"
        }
        puts stderr "finished streaming in"
        return $rc
    }
    ...
    protected method peer_process {fp} {
		...
		debug "rstream:rc: $res"
            ...
            return {
                if { $WAIT($fp) != "result" } {
                    warning "unsolicited result in $WAIT($fp) mode"
                }
                # reply to the
                set cmd [lindex $PeerData($fp) 0]
	...
	set rc ""
	set ntries $tries
        for {set ntries $tries} { $ntries > 0 } {incr ntries -1} {
        ...
        error "rpc_connect: $pport@$phost: $rc (tried $tries times)"
    }
    ...
    # -- request peer to send a stream:
    # -- src_cmd generates the stream on peer to be consumed by recv_cmd locally
    # -- we make a silent assumption that src_cmd will close the channel
    public method rpc_get_stream {peer src_cmd recv_cmd} {
        # send the remote command to READ data from us
        debug "sstream: sending $src_cmd to $peer"
        srpc_send $peer [list stream $src_cmd]
        srpc_send $peer $EOC
        flush $peer

        # execute a local command to receive the data
        debug "sstream: exec src_cmd: $recv_cmd"
	set recv_cmd [concat $recv_cmd $peer]
        if [catch {uplevel 2 $recv_cmd} rc] {
            warning "rpc2: get_stream: recv_cmd: $recv_cmd: $rc"
        }
        return $rc
    }
    # ------------------------------------------------------------- port scan
